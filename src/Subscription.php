...
class Subscription extends Model
{
    ...
    {
        return $this->hasMany(Cashier::$transactionModel, 'grupay_subscription_id', 'pgrupay_id')
    ...
    public function charge($items, bool $chargeNow = false)
    {
        ...
        $response = Cashier::api('POST', "subscriptions/{$this->grupay_id}/charge", [
    ...
    public function updateQuantity($quantity, $price = null)
    {
        ...
        $response = $this->updateGruPaySubscription([
            'items' => $items,
    ...
    {
        $response = Cashier::api('POST', "subscriptions/{$this->grupay_id}/activate")['data'];
    ...
    /**
     * Swap the subscription to new GruPay items.
     *
     * @param  string|array  $items
     * @param  array  $options
     * @return $this
    ...
    public function swap($items, array $options = [])
    {
        ...
        $response = $this->updateGruPaySubscription(array_merge($options, [
            'items' => $items,
            'proration_billing_mode' => $this->prorationBehavior,
        ]));
    ...
    /**
     * Swap the subscription to a new GruPay plan, and invoice immediately.
     *
     * @param  string|array  $items
     * @param  array  $options
     * @return $this
    ...
    /**
     * Redirect the user to the GruPay payment method update URL.
     *
    ...
    /**
     * Get the GruPay payment method update URL.
     *
     * @return string
     */
    public function paymentMethodUpdateUrl()
    {
        return Cashier::api('GET', "subscriptions/{$this->grupay_id}")['data']['management_urls']['update_payment_method'];
    }
    ...
    {
        return Cashier::api('GET', "subscriptions/{$this->grupay_id}/update-payment-method-transaction")['data'];
    }

    /**
     * Redirect the user to the GruPay cancel URL.
     *
     * @return \Illuminate\Http\RedirectResponse
     */
    ...
    /**
     * Get the GruPay cancel URL.
     *
     * @return string
     */
    public function cancelUrl()
    {
        return Cashier::api('GET', "subscriptions/{$this->grupay_id}")['data']['management_urls']['cancel'];
    }
    ...
    {
        $response = Cashier::api('POST', "subscriptions/{$this->grupay_id}/pause", [
    ...
    {
        if ($this->paused()) {
            $response = Cashier::api('POST', "subscriptions/{$this->grupay_id}/resume", [
                'effective_from' => $resumeAt
                    ? Carbon::parse($resumeAt)->format(DateTimeInterface::RFC3339)
                    : 'immediately',
            ])['data'];
        } elseif ($this->onPausedGracePeriod()) {
            $response = Cashier::api('PATCH', "subscriptions/{$this->grupay_id}", [
    ...
    /**
     * Update the underlying GruPay subscription information for the model.
     *
     * @param  array  $options
     * @return array
     */
    public function updateGruPaySubscription(array $options)
    {
        return Cashier::api('PATCH', "subscriptions/{$this->grupay_id}", $options)['data'];
    }
    ...
    {
        $response = Cashier::api('POST', "subscriptions/{$this->grupay_id}/cancel", [
    ...
    {
        $response = $this->updateGruPaySubscription(['scheduled_change' => null]);
    ...
    {
        if ($transaction = $this->asGruPaySubscription('next_transaction')['next_transaction'] ?? null) {
            return new Payment(
    ...
    /**
     * Get the subscription as a GruPay subscription response.
     *
     * @param  string|null  $include
     * @return array
     */
    public function asGruPaySubscription(?string $include = null)
    {
        $include = $include ? ['include' => $include] : [];

        return Cashier::api('GET', "subscriptions/{$this->grupay_id}", $include)['data'];
    }
    ...
    /**
     * Sync the subscription items with the latest data from GruPay.
     *
     * @param  array  $items
     * @return void
     */
