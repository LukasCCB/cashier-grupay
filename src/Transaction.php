...
class Transaction extends Model
{
    ...
    public function invoicePdf ()
    {
        ...
        return Cashier::api('GET', "transactions/{$this->grupay_id}/invoice")['data']['url'] ?? null;
    }
    ...
    public function adjust ($type, $reason, $prices = [])
    {
        ...
        if (empty($items)) {
            ...
            throw new Exception(
                "Cannot find line items with price ID's `{$prices}` for transaction `{$this->grupay_id}`."
            );
        }

        return Cashier::api('POST', 'adjustments', [
            'action' => $type,
            'transaction_id' => $this->grupay_id,
            'reason' => $reason,
            'items' => $items,
        ])['data'];
    }
    ...
    /**
     * Get the transaction as a GruPay transaction response.
     *
     * @return array
     */
